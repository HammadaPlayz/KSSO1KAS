...
client.on('message' , async (message) => {
  ...
  if (args[0] === undefined) {
    ...
    .setColor("FFFFFF")
    .setDescription("من فضلك قم بكتابه لغه");
    ...
    if (args[1] === undefined) {
      ...
      return message.channel.send('**من فضلك قم بكتابه كلمه`');
...
(edited)
 args = args.join(' ').slice(prefix.length);
      ...
      if (!Langs.includes(transArg)) return message.channel.send(`**لا يوجد لغه بـهذه الاسم**`);
