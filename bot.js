...
const item = type[Math.floor(Math.random() * type.length)];
const answer = (answer => answer.toLowerCase() === response.content.toLowerCase());
const filter = response => { 
    return item.answers.some(answer => answer.toLowerCase() === response.content.toLowerCase());
};
message.channel.send('**لديك 15 ثانيه لتعرف الاجابه**').then(msg => {
    let embed = new Discord.RichEmbed()
    .setColor('#000000')
    .setFooter("معلومات ماينكرفت  | NoobBot", 'https://cdn.discordapp.com/avatars/439427357175185408/3eb163b7656922ebc9e90653d50231f1.png?size=2048')
    .setDescription(`** ${item.type}**`)
    ...
    msg.channel.sendEmbed(embed).then(() => {
        ...
        console.log(`[Typing] ${collected.first().author} typed the word.`);
          ...
          .catch(collected => { 
            message.channel.send(`:x: **لا يوجد احد كتب الاجابه الصحيحه والاجابه هي ${item.answer}**`);
            console.log(`[Typing] playing`);
          })
        })
    })
}
