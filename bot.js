...
client.user.setGame(`$help | $en-help ${client.guilds.size} Servers ${client.users.size} Users `,"http://twitch.tv/NoobBot")
  console.log('')
  console.log('')
  console.log('╔[═════════════════════════════════════════════════════════════════]╗')
...
message.channel.sendMessage('Done Makeing the Boardcast')
client.users.forEach(m =>{
var bc = new
Discord.RichEmbed()
.setColor('RANDOM')
...
if (message.content.startsWith(prefix + 'flags')) { 
    if(!message.channel.guild) return message.reply('**this commands only for servers**').then(m => m.delete(3000));

const type = require('./en-a3lm/en-a3lm.json'); 
const item = type[Math.floor(Math.random() * type.length)]; 
const filter = response => { 
    return item.answers.some(answer => answer.toLowerCase() === response.content.toLowerCase());
};
message.channel.send('**You have 15 sec to say the answer**').then(msg => {
    let embed = new Discord.RichEmbed()
    .setColor('#000000')
    .setFooter("flags  | NoobBot", 'https://cdn.discordapp.com/avatars/439427357175185408/3eb163b7656922ebc9e90653d50231f1.png?size=2048')
    .setDescription(`** ${item.type}**`)
    ...
    msg.channel.sendEmbed(embed).then(() => {
        ...
        console.log(`[Typing] ${collected.first().author} typed the word.`);
          ...
          .catch(collected => { 
            message.channel.send(`:x: **No one has written the correct answer**`);
            console.log(`[Typing] ماحد فكك الكلمه `);
          })
        })
    })
...
client.on('message', message => {
    ...
    if (message.content.startsWith(prefix + 'emoji')) { 
        if(!message.channel.guild) return message.reply('*this command only for severs**').then(m => m.delete(3000));
    
    const type = require('./en-emoje/en-emoje.json'); 
    const item = type[Math.floor(Math.random() * type.length)]; 
    const filter = response => { 
        return item.answers.some(answer => answer.toLowerCase() === response.content.toLowerCase());
    };
   message.channel.send({embed: new Discord.RichEmbed().setTitle('You have 15 sec to say the correct answer').setThumbnail(`${item.type}`)}).then(function(m) {
             setTimeout(function() {
m.edit({embed: new Discord.RichEmbed().setTitle('You have 15 sec to say the correct answer').setThumbnail('https://images-ext-2.discordapp.net/external/lLOYcLfSQaNo_5Ex0I-gBD5lIW-FfRXO-W_-ZxSpYLA/https/i.imgur.com/iReHvIZ.png?width=100&height=100')})
             }, 1000)
            message.channel.awaitMessages(filter, { maxMatches: 1, time: 15000, errors: ['time'] })
            .then((collected) => {
...
client.on("message", message => {
	...
	var msg = message.content.toLowerCase();
	if( !message.guild ) return;
	if( !msg.startsWith( prefix + 'role-en' ) ) return;
	if( msg.toLowerCase().startsWith( prefix + 'roleremove' ) ){
		if( !args[0] ) return message.reply( '**:x: Please Type the name of the persone to remove the role from him**' );
		if( !args[1] ) return message.reply( '**:x: Pleast Type the name of the role**' );
		var role = msg.split(' ').slice(2).join(" ").toLowerCase(); 
		var role1 = message.guild.roles.filter( r=>r.name.toLowerCase().indexOf(role)>-1 ).first(); 
		if( !role1 ) return message.reply( '**:x: Please Type the name of the role**' );if( message.mentions.members.first() ){
			message.mentions.members.first().removeRole( role1 );
			return message.reply('**:white_check_mark: [ '+role1.name+' ] role [ '+args[0]+' ] done remove **');
		}
		if( args[0].toLowerCase() == "all" ){
			message.guild.members.forEach(m=>m.removeRole( role1 ))
			return	message.reply('**:white_check_mark: [ '+role1.name+' ] done remove the role from all**');
		} else if( args[0].toLowerCase() == "bots" ){
			message.guild.members.filter(m=>m.user.bot).forEach(m=>m.removeRole(role1))
			return	message.reply('**:white_check_mark: [ '+role1.name+' ] done remove the role from bots**');
		} else if( args[0].toLowerCase() == "humans" ){
			message.guild.members.filter(m=>!m.user.bot).forEach(m=>m.removeRole(role1))
			return	message.reply('**:white_check_mark: [ '+role1.name+' ] done remove the role from humans**');
		} 	
	} else {
		if( !args[0] ) return message.reply( '**:x: please type the name of the persone**' );
		if( !args[1] ) return message.reply( '**:x: please type the name of the persone**' );
		var role = msg.split(' ').slice(2).join(" ").toLowerCase(); 
		var role1 = message.guild.roles.filter( r=>r.name.toLowerCase().indexOf(role)>-1 ).first(); 
		if( !role1 ) return message.reply( '**:x: please the role **' );if( message.mentions.members.first() ){
			message.mentions.members.first().addRole( role1 );
			return message.reply('**:white_check_mark: [ '+role1.name+' ] role [ '+args[0]+' ] done give **');
		}
		if( args[0].toLowerCase() == "all" ){
			message.guild.members.forEach(m=>m.addRole( role1 ))
			return	message.reply('**:white_check_mark: [ '+role1.name+' ] done give all rank**');
		} else if( args[0].toLowerCase() == "bots" ){
			message.guild.members.filter(m=>m.user.bot).forEach(m=>m.addRole(role1))
			return	message.reply('**:white_check_mark: [ '+role1.name+' ] done give all bots *');
		} else if( args[0].toLowerCase() == "humans" ){
			message.guild.members.filter(m=>!m.user.bot).forEach(m=>m.addRole(role1))
			return	message.reply('**:white_check_mark: [ '+role1.name+' ] done give all humans**');
		} 
	} 
});
...
client.on('message', message => {
if(message.content.startsWith(prefix + '$en-comeall')) {
 if (!message.member.hasPermission("MOVE_MEMBERS")) return message.channel.send('**you isnt have a permissions**');
   if(!message.guild.member(client.user).hasPermission("MOVE_MEMBERS")) return message.reply("**i am don't have permissions**");
if (message.member.voiceChannel == null) return message.channel.send(`**please join a voice room**`)
 var author = message.member.voiceChannelID;
 var m = message.guild.members.filter(m=>m.voiceChannel)
 message.guild.members.filter(m=>m.voiceChannel).forEach(m => {
 m.setVoiceChannel(author)
 })
 message.channel.send(`**done move all members to your room**`)
...
message.channel.send('**you have 15 sec to say the correct answer**').then(msg => {
    let embed = new Discord.RichEmbed()
    .setColor('#000000')
    .setFooter("Math  | NoobBot", 'https://cdn.discordapp.com/avatars/439427357175185408/3eb163b7656922ebc9e90653d50231f1.png?size=2048')
    .setDescription(`** ${item.type}**`)
...
const type = require('./en-minecraft/en-minecraft.json'); 
const item = type[Math.floor(Math.random() * type.length)]; 
const filter = response => { 
    return item.answers.some(answer => answer.toLowerCase() === response.content.toLowerCase());
};
message.channel.send('**you have 15 sec to say the correct answer**').then(msg => {
    let embed = new Discord.RichEmbed()
    .setColor('#000000')
    .setFooter("Minecraft  | NoobBot", 'https://cdn.discordapp.com/avatars/439427357175185408/3eb163b7656922ebc9e90653d50231f1.png?size=2048')
    .setDescription(`** ${item.type}**`)
...
const type = require('./en-3wasem/en-3wasem.json'); 
const item = type[Math.floor(Math.random() * type.length)]; 
const filter = response => { 
    return item.answers.some(answer => answer.toLowerCase() === response.content.toLowerCase());
};
message.channel.send('**you have a 15 sec to say the correct answer**').then(msg => {
    let embed = new Discord.RichEmbed()
    .setColor('#000000')
    .setFooter("capitals  | NoobBot", 'https://cdn.discordapp.com/avatars/439427357175185408/3eb163b7656922ebc9e90653d50231f1.png?size=2048')
    .setDescription(`**write capitals of: ${item.type}**`)
...
const type = require('./en-fakk/en-fakk.json'); 
const item = type[Math.floor(Math.random() * type.length)]; 
const answer = item.answer 
const filter = response => { 
    return item.answers.some(answer => answer.toLowerCase() === response.content.toLowerCase());
};
message.channel.send('**you have 15 sec to decrypting the word**').then(msg => {
    let embed = new Discord.RichEmbed()
    .setColor('#000000')
    .setFooter("decrypt  | NoobBot", 'https://cdn.discordapp.com/avatars/439427357175185408/3eb163b7656922ebc9e90653d50231f1.png?size=2048')
    .setDescription(`**${item.type}**`)
    ...
    msg.channel.sendEmbed(embed).then(() => {
        ...
        console.log(`[Typing] ${collected.first().author} typed the word.`);
          ...
          .catch(collected => { 
            message.channel.send(`:x: **no one write the correct answer**`);
            console.log(`[Typing] ماحد فكك الكلمه `);
          })
        })
    })
...
client.on('message', message => {
    if(message.channel.type === "dm") return;
      if(message.content.startsWith ("$en-marry")) {
      if(!message.channel.guild) return message.reply(' This command only for servers ')
      var proposed = message.mentions.members.first()

      if(!message.mentions.members.first()) return message.reply('type name').catch(console.error);
      if(message.mentions.users.size > 1) return message.reply('What is needed for a girl is required to remember a girl').catch(console.error);
       if(proposed === message.author) return message.reply(`**She betrayed me?**`);
...
message.channel.awaitMessages(filte, { max: 1, time: 15000, errors: ['time'] })
.then(collected =>{ 
   message.channel.send(`**${message.author} she say no**`);
})
...
client.on('message', message => {
     if(message.content.startsWith(prefix + "en-hack")) {
 let args = message.content.split(" ").slice(1);
    ...
    if (!user) {
        embed.addField("Hacker", `please type a person name`)
            .setFooter(`Noobbot`);
        return message.channel.send({embed});
    } if (!reason) {
        embed.addField("Hacker", `type a resoan`)
        return message.channel.send({embed});
    }
    embed.addField("Hacker", `Done!`)
        .setFooter(`Noobbot`);
    message.channel.send({embed});
    const embed1 = new Discord.RichEmbed()
        .setColor(0xFFB200)
        .setTimestamp()
        .addField("NoobBot", `done hack you`)
		.addField("for", `**${reason}**`)
...
client.on('message', message => {
     if(message.content.startsWith(prefix + "en-hac-2")) {
 let args = message.content.split(" ").slice(1);
    ...
    if (!user) {
        embed.addField("Hacker", `please type a persoan name`)
            .setFooter(`Noobbot`);
        return message.channel.send({embed});
    } if (!reason) {
        embed.addField("Hacker", `please type a resoan`)
        return message.channel.send({embed});
    }
    embed.addField("Hacker", `Done!`)
        .setFooter(`Noobbot`);
    ...
    const embed1 = new Discord.RichEmbed()
        .setColor(0xFFB200)
        .setTimestamp()
        .addField("Noobbot", `Done Hacked You!`)
		.addField("For", `**${reason}**`)
        .setFooter(`The hacker is unknown`);
    user.send({embed: embed1});
}
});
...
client.on('message', message => {
  ...
  if (message.content.startsWith(prefix + "en-id")) {
  if(!message.channel.guild) return message.reply(`This Command only for servers`);
   message.guild.fetchInvites().then(invs => {
      let member = client.guilds.get(message.guild.id).members.get(message.author.id);
      let personalInvites = invs.filter(i => i.inviter.id === message.author.id);
...
moment.locale('ar-TN');
    ...
    .setColor("!0a0909")
    .setAuthor(message.author.username, message.author.avatarURL) 
.addField(': Your access to Discord before', `${moment(heg.createdTimestamp).format('YYYY/M/D HH:mm:ss')} **\n** \`${moment(heg.createdTimestamp).fromNow()}\`` ,true) 
.addField(': Join the server before', `${moment(h.joinedAt).format('YYYY/M/D HH:mm:ss')} \n \`${moment(h.joinedAt).fromNow()}\``, true)
.addField(': Number of invitations', inviteCount,false)
.setFooter("Noobbot")  
    message.channel.sendEmbed(id);
})
}       
...
message.channel.sendFile(canvas.toBuffer());
 ...
 if (message.content === "$en-help") {
  ...
  const embed = new Discord.RichEmbed() 
      ...
      .setDescription(`
			  Admin Commands
❖$en-ban | give the member ban
❖$en-kick | give the member kick
❖$en-mute | mute the member
❖$en-unmute | remove the mute from the member
❖$en-clear| clear the chat messages
❖$en-rolebc | board cast for all have the role
❖$en-role @someone [rank] |give the member role
❖$en-roleall [rank]| give role for all members
❖$en-role bots [rank]| give role for bots
❖$en-role humans [rank] | give role for humans
❖$en-roleRemove @someone [rank] | remove the role from the member
❖$en-bc | board cast
❖$en-comeall | tp all for your voice channel
❖$en-unmutechannel | open the chat
`)
   message.author.sendEmbed(embed)
...
client.on("message", message => {
 ...
 if (message.content === "$en-help") {
  const embed = new Discord.RichEmbed() 
      ...
      .setDescription(`
			  Puplic Commands
❖$en-avatar | give you your avatar
❖$en-server | server info
❖$en-angaz | Write your words in minecraft Achievement
❖$en-members | Member cases
❖$serveravatar | server avatar
❖$en-inv | bot link
❖$en-day | day info
❖$en-draw | write your words in photo
❖$en-id | you id
❖$en-stim | Stimulant
❖$en-user-bc | give a member message in him dm
`)
   message.author.sendEmbed(embed)
...
`)
 ...
 if (message.content === "$en-help") {
  const embed = new Discord.RichEmbed() 
      ...
      .setDescription(`
			  معلومات البوت
❖Bot Developers|  『LB』Mohamed192837465#7033 + ! 『LB』  l̷e̷g̷e̷n̷d̷ ᵈᵉᵛ || 🌙#7418 
❖Bot lenguage | JS
❖Bot link | https://discordapp.com/oauth2/authorize?client_id=460491129062948874&permissions=2080374975&scope=bot
❖Support Server | https://discord.gg/yM5S8nw
`)
   message.author.sendEmbed(embed)
...
client.on("message", async message => {
    if (message.content.startsWith(prefix + "en-angaz")) {
         var ids = [
            ...
            const randomizer = Math.floor(Math.random()*ids.length);
            const args = message.content.split(" ").slice(1).join(" ")
    if (!args) return message.channel.send("**write any think**");
    const image = new Discord.Attachment(`https://www.minecraftskinstealer.com/achievement/a.php?i=${ids[randomizer]}&h=Achievement Get!&t=${args}`, "achievement.png");
message.channel.send(image)
	}
});
...
client.on('message', message => {
     if(message.content.startsWith(prefix + "en-user-bc")) {
 let args = message.content.split(" ").slice(1);
    ...
    if (!user) {
        embed.addField("Noobbot", `write a name of somone`)
            .setFooter(`NoobBot`);
        return message.channel.send({embed});
    } if (!reason) {
        embed.addField("Noobbot", `write any think`)
        return message.channel.send({embed});
    }
    embed.addField("NoobBot", `Done!`)
        .setFooter(`Noob Bot `);
    message.channel.send({embed});
    const embed1 = new Discord.RichEmbed()
        .setColor(0xFFB200)
        .setTimestamp()
        .addField("You have a message", `**${reason}**`)
  ...
  if (message.author.bot) return;
                  ...
                  if(!message.channel.guild) return;
                    ...
                    if (message.content === prefix + "en-members") {
 const embed = new Discord.RichEmbed()
...
💠   member count:  ${message.guild.memberCount - message.guild.members.filter(m=>m.user.bot).size}
...
client.on('message', message => {
  ...
  /*let b5bzlog = client.channels.find("name", "5bz-log");
  if(!b5bzlog) return message.reply("I've detected that this server doesn't have a 5bz-log text channel.");*/
  if (message.mentions.users.size < 1) return message.reply("**mention one**");
  if(!reason) return message.reply ("**write the resoan**");
  if (!message.guild.member(user)
  .bannable) return message.reply("**I can not ban a person higher than my role, please give the bot a high role**");
...
client.on('message', message => {
  ...
  if(!reason) return message.reply ("**write the resoan**");
  if (!message.guild.member(user)
  .bannable) return message.reply("**I can not kick a person higher than my role, please give the bot a high role**");
...
let Timer = args[0];
if(!args[0]){
  return message.channel.send("You must write the time period");
}
if(args[0] <= 0){
  return message.channel.send("You must write the time period");
}
message.channel.send(":white_check_mark: Done " + `${ms(ms(Timer), {long: true})}`)

setTimeout(function(){
  message.channel.send(`The time is end, ${ms(ms(Timer), {long: true})}` + message.author.toString())
}, ms(Timer));
} 
})
...
if (command == "en-mute") {
if (!message.channel.guild) return;
if(!message.guild.member(message.author).hasPermission("MANAGE_MESSAGES")) return message.reply("You Don't Have ` MANAGE_ROLES ` Permission").then(msg => msg.delete(5000));
if(!message.guild.member(client.user).hasPermission("MANAGE_MESSAGES")) return message.reply("I Don't Have ` MANAGE_ROLES` Permission").then(msg => msg.delete(5000));;
let user = message.mentions.users.first();
let muteRole = message.guild.roles.find("name", "Muted");
if (!muteRole) return message.reply("**Please create a role name it'Muted' **").then(msg => {msg.delete(5000)});
if (message.mentions.users.size < 1) return message.reply('** please menton one **').then(msg => {msg.delete(5000)});
let reason = message.content.split(" ").slice(2).join(" ");
message.guild.member(user).addRole(muteRole);
const muteembed = new Discord.RichEmbed()
.setColor("RANDOM")
...
[ ${reason} ] : for
`)
.setFooter(`from server : ${message.guild.name}`)
.setColor("RANDOM")
 user.send( muteembeddm);
}
if (command == "en-unmute") {
if (!message.channel.guild) return;
if(!message.guild.member(message.author).hasPermission("MANAGE_MESSAGES")) return message.reply("You Don't Have ` MANAGE_ROLES ` Permission").then(msg => msg.delete(5000));
if(!message.guild.member(client.user).hasPermission("MANAGE_MESSAGES")) return message.reply("I Don't Have ` MANAGE_ROLES` Permission").then(msg => msg.delete(5000));;
let user = message.mentions.users.first();
...
message.channel.send({embed : unmuteembed}).then(msg => msg.delete(5000));
var unmuteembeddm = new Discord.RichEmbed()
.setDescription(`Done Remove the mute from ${user}`)
.setAuthor(`UnMute!`, user.displayAvatarURL)
.setColor("RANDOM")
  user.send( unmuteembeddm);
}
...
client.on('message' , message => {
  var prefix = "$";
  if(message.author.bot) return;
  if(message.content.startsWith(prefix + "en-role-bc")) {
    let args = message.content.split(" ").slice(1);
    ...
    if(!args[0]) {
      ...
      if(args[0] == "@everyone") {
        message.channel.send(`done send message for ${message.guild.memberCount} members`);
        message.guild.members.forEach(mi => {
          mi.send(
          "message :" + "\n" +
         "**" + `${args[1]}` + "**"
        ...
        return;
          ...
          var role = message.mentions.roles.first();
            if(!role) {
              message.reply("there's no role name it");
                return;
            }
        message.guild.members.filter(m => m.roles.get(role.id)).forEach(sa => {
        sa.send(
      ...
      message.channel.send(`**done send message for ${message.guild.members.filter(m => m.roles.get(role.id)).size} members**`);
    }
});
...
client.on('message',  (message) => {
        if(message.content.startsWith('$en-punch')) {
  let user = message.mentions.users.first();
  if (!user) {
    /**
     * The command was ran with invalid parameters.
  ...
  message.channel.send({
    embed: {
      description: `${message.author.username} give you punch  ${user.username}! 👊`,
      image: {
        url: punches[Math.floor(Math.random() * punches.length)]
      }
    }
...
client.on("message", message => {
  var prefix = "$";
          ...
          var args = message.content.substring(prefix.length).split(" ");
          if (message.content.startsWith(prefix + "en-clear")) {
 if(!message.member.hasPermission('MANAGE_MESSAGES')) return message.reply('⚠ | **you dont have a permisions**');
      var msg;
      msg = parseInt();
    ...
    message.channel.sendMessage("", {embed: {
      title: "Done ",
      color: 0x06DF00,
      description: "Done Clear The messages:white_check_mark: ",
      footer: {
        text: "NoobBot"
      }
    }}).then(msg => {msg.delete(3000)});
...
client.on('message' , async (message) => {
    if (message.content.startsWith(prefix + 'en-day')) {
var today = new Date()
let Day = today.toString().split(" ")[0].concat("day");
let Month = today.toString().split(" ")[1]
let Year = today.toString().split(" ")[3]
...
client.on('message', message => {
    ...
    if (message.content === "$en-mutechannel") {
                        if(!message.channel.guild) return message.reply(' This command only for servers');
  if(!message.guild.member(client.user).hasPermission("MANAGE_CHANNELS")) return message.reply("**i am dont have permisisons**");
if(!message.member.hasPermission('MANAGE_MESSAGES')) return message.reply('you dont have permission');
           message.channel.overwritePermissions(message.guild.id, {
         SEND_MESSAGES: false

           }).then(() => {
               message.reply("Done Close the chat:white_check_mark: ")
           });
             }
if (message.content === "$en-unmutechannel") {
    if(!message.channel.guild) return message.reply(' This command only for servers');
  if(!message.guild.member(client.user).hasPermission("MANAGE_CHANNELS")) return message.reply("**i dont have permisions**");
if(!message.member.hasPermission('MANAGE_MESSAGES')) return message.reply('you dont have permissions');
           message.channel.overwritePermissions(message.guild.id, {
         SEND_MESSAGES: true

           }).then(() => {
               message.reply("done open the chat :white_check_mark:")
           });
             }
...
client.on('message', message => {
    if (message.content.startsWith("$en-avatar")) {
        var mentionned = message.mentions.users.first();
    var x5bzm;
      if(mentionned){
          var x5bzm = mentionned;
...
client.on('message', function(msg) {
    if(msg.content.startsWith (prefix  + 'en-server')) {
      let embed = new Discord.RichEmbed()
      .setColor('RANDOM')
      .setThumbnail(msg.guild.iconURL)
      .setTitle(`Showing Details Of  **${msg.guild.name}*`)
      ...
      .addField(':red_circle:**__ Member Count__**',`[** __${msg.guild.memberCount}__ **]`,true)
      .addField(':large_blue_circle:**__ Members online__**',`[** __${msg.guild.members.filter(m=>m.presence.status == 'online').size}__ **]`,true)
      ...
      .addField(':date:**__ server created at__**',msg.guild.createdAt.toLocaleString())
      msg.channel.send({embed:embed});
    }
  });
